...
def run_gimp_mask_a_batch(runtime, gimp_executable_path: Path, gimp_batch_profile_path: Path, mask_jobs: list[tuple[Path, Path]]) -> str:
    if not mask_jobs:
        runtime.fail(runtime, 'Es wurden keine Mask-A-Bildaufträge übergeben.')
    batch_script = runtime.build_gimp_mask_a_batch_script(runtime, mask_jobs)
    ...
    if result.returncode != 0:
        runtime.fail(runtime, f'GIMP konnte die Mask-A-PNGs nicht erzeugen.\nExit-Code: {result.returncode}\nStandardausgabe:\n{result.stdout.strip()}\nFehlerausgabe:\n{result.stderr.strip()}')
    return portable_gimp_status

def register(runtime):
    runtime.run_gimp_mask_a_batch = run_gimp_mask_a_batch
